resource "vsphere_virtual_machine" "rosg" {
...
#      "CheckPoint.ftwSicKey" = "vpn123"
      "user_data" = "${base64encode(<<EOF
          #cloud-config
          hostname: vpn-gw7${count.index + 1}
          ...
          runcmd:
            ...
            - clish -s -c 'set interface eth0 ipv6-address 2a04:6447:900:500::7${count.index + 1} mask-length 64'
            - clish -s -c 'set interface eth1.100 ipv6-address 2a04:6447:900:100::7${count.index + 1} mask-length 64'
            - clish -s -c 'set interface eth1.101 ipv6-address 2a04:6447:900:101::7${count.index + 1} mask-length 64'
            - clish -s -c 'set interface eth1.102 ipv6-address 2a04:6447:900:102::7${count.index + 1} mask-length 64'
            - clish -s -c 'set interface eth2.200 ipv6-address 2a04:6447:900:200::7${count.index + 1} mask-length 64'
#          bootcmd:
          clish:
...
#          blink_config:
          network:
            ...
            - type: physical
              ...
              subnets:
                - type: static
                  address: 192.168.233.7${count.index + 1}/24
...
#                  dns_nameservers: [192.168.233.233, 8.8.8.8]
            ...
            - type: vlan
              ...
              subnets:
                - type: static
                  address: 10.0.100.7${count.index + 1}/24
            - type: vlan
              name:      eth1.101
              vlan_link: eth1
              vlan_id:   101
              subnets:
                - type: static
                  address: 10.0.101.7${count.index + 1}/24
            - type: vlan
              name:      eth1.102
              vlan_link: eth1
              vlan_id:   102
              subnets:
                - type: static
                  address: 10.0.102.7${count.index + 1}/24
            - type: physical
              name: eth2
            - type: vlan
              name:      eth2.200
              ...
              subnets:
                - type: static
                  address: 10.0.20.7${count.index + 1}/24
            EOF
      )}" 
    }
  }
