  ...
  vapp {
    properties = {
      ...
      "user_data" = "${base64encode(<<EOF
          #cloud-config
          hostname: lab-ro-sg7${count.index + 1}
          runcmd:
...
#            - set user admin password password-hash ${var.chkp_passwd_hash} # Does not work..
          ...
          network:
            ...
            - type: physical
              ...
              subnets:
                - type: static
                  address: 1.1.2.7${count.index + 1}/30
            - type: physical
              name: eth2
              mtu: 9000
              subnets:
                - type: static
                  address: 2.2.2.7${count.index + 1}/24
            - type: physical
              name: eth3
              mtu: 9000
              subnets:
                - type: static
                  address: 3.3.3.7${count.index + 1}/24
            EOF
      )}" 
    }
  }
